...
phrases. Each is skipped — not failed — when not applicable. auto_ingest is
forced off so the eval never modifies the corpus.

    python -m scripts.eval                       # with generation (default)
...
def run_question(
    pipeline: RAGPipeline, q: Dict[str, Any], skip_generation: bool
) -> QuestionOutcome:
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    for i, q in enumerate(questions, start=1):
        ...
        try:
            outcomes.append(run_question(pipeline, q, skip_generation))
